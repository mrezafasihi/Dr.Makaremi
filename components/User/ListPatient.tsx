  ...
  const confirm = (data: any) => {
    if (selectedPatiend?.id == undefined) {
      ...
      apiRequests
        ...
        .then((res) => {
          localStorage.setItem("selectedPatient",JSON.stringify(res.data.data))
          console.log(res)
          setOpenModal(true);
          setTimeout(() => {
            setOpenModal(false);
      ...
      console.log(selectedPatiend)
      
      localStorage.setItem("selectedPatient",JSON.stringify(selectedPatiend))
      route.push("/user/reservation/time");
    }
  };
